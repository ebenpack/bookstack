...
import { CategoryRecord, CategoryDetailRecord } from '../Category/categoryModule';
...
export const makeBook = (book: BookParams): BookRecord =>
    new BookRecord({
        ...book,
        authors: List(book.authors ? book.authors.map(author => new AuthorRecord(author)) : []),
        publishers: List(book.publishers ? book.publishers.map(publisher => new PublisherRecord(publisher)) : [])
    });
...
export const makeBookstack = (bookStack: BookStackParams): BookStackRecord =>
    new BookStackRecord({
        ...
        categories: List(bookStack.categories ? bookStack.categories.map(makeCategory): [])
    })